{* $Id: header.tpl 78633 2021-07-12 12:00:50Z bsfez $ *}
...
{else}
    ...
    {else}
        ...
        {else}
            ...
            {elseif !empty($page)}
                {$page|escape} {* Must be tested if description is empty (not before) *}
            {elseif !empty($headtitle)}
                {$headtitle|stringfix:"&nbsp;"|escape}{* use $headtitle last if no other candidate is found *}
            {/if}
        {/if}
    {/if}
